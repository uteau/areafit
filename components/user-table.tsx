...
export function UserTable({ users }: { users: (Profile & { email: string })[] }) {
  if (users.length === 0) {
    return <p className="text-sm font-medium text-lit/60">Aún no hay jugadores</p>
  }

  return (
    <>
      {/* Móvil: tarjeta por jugador */}
      <div className="space-y-3 md:hidden">
        {users.map((user) => (
          <div key={user.id} className="panel p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="truncate font-bold text-lit">{user.full_name}</p>
                <p className="mt-0.5 truncate text-sm text-lit/60">{user.email}</p>
              </div>
              <span className="pill shrink-0">{labelOf(user.role)}</span>
            </div>
            <div className="mt-3 border-t border-seam pt-3">
              <RoleForm userId={user.id} currentRole={user.role} fullWidth />
              <div className="mt-2">
                <ConfirmDelete
      ...
      <div className="panel hidden overflow-x-auto md:block">
        <table className="w-full min-w-[560px] text-left text-sm">
          <thead>
            <tr className="border-b border-seam text-[11px] font-bold uppercase tracking-[0.14em] text-lit/60">
